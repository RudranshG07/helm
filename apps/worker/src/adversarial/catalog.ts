...
export const SCENARIOS: Scenario[] = [
  ...
  { id: 'J1', category: 'Not yet built', title: 'No live gateway implementation exists', expectation: 'A real Razorpay client that creates orders and charges invoices.', outcome: 'UNHANDLED', note: 'The only gateway is RefusingGateway, which throws on every charge. Every exactly-once guarantee is proven against a stub. Nothing has been proven against Razorpay itself, and the first live call is untested code.' },
  { id: 'J2', category: 'Not yet built', title: 'Downtime webhooks from the gateway are never consumed', expectation: 'payment.downtime.started and .resolved feed the degradation signal.', outcome: 'UNHANDLED', note: 'recordRazorpayDowntime exists and is tested, but no webhook route or poller calls it. In production only our own rollup would ever fire, so the lead-time comparison has nothing to compare against.' },
